...
\begin{alltt}
\hlkwd{require}\hlstd{(knitr)}  \hlcom{# Needed for write_bib()}

\hlcom{# Load some packages to the session:}
...
{\ttfamily\noindent\color{warningcolor}{\#\# Warning: there is no package called 'xtable'}}\begin{alltt}
...
{\ttfamily\noindent\color{warningcolor}{\#\# Warning: there is no package called 'ggplot2'}}\begin{alltt}
\hlcom{# Select packages to cite:}
\hlstd{citPkgs} \hlkwb{<-} \hlkwd{names}\hlstd{(}\hlkwd{sessionInfo}\hlstd{()}\hlopt{$}\hlstd{otherPkgs)}
\hlcom{# Write the bibtex file:}
\hlkwd{write_bib}\hlstd{(citPkgs,} \hlkwc{file} \hlstd{=} \hlstr{"journal.bib"}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
